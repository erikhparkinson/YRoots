...
SubdivisionSolver<D>::SubdivisionSolver(const std::vector<std::unique_ptr<FunctionInterface>>& _functions, tbb::strict_ppl::concurrent_queue<SolveParameters>& _intervalsToRun, SubdivisionParameters _parameters, IntervalTracker& _intervalTracker, RootTracker& _rootTracker) :
...
m_intervalChecker(m_intervalTracker, m_rank, m_intervalsToRun),
m_linearSolver(m_rank, m_intervalTracker, m_rootTracker)
{
...
void SubdivisionSolver<D>::solve(SolveParameters& _parameters)
{
    ...
    {
        m_intervalTracker.storeResult(_parameters.interval, SolveMethod::TooDeep);
        return;
    }
